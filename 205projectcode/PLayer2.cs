...
public class PLayer2 : MonoBehaviour {
    ...
    void Start()
    {
        ...
        topDownCamera = GameObject.Find("Main Camera Player2").GetComponent<Camera>();
    ...
    void FixedUpdate()
    {
		...
		if (Input.GetKeyDown (KeyCode.KeypadPeriod) && ammo > 0) {
        ...
        if (Input.GetKey(KeyCode.RightShift) && !tired) {
            sprinting = true;
        ...
        if (Input.GetKeyDown(KeyCode.Keypad0) && !currentFloor.Equals(Floor.BottomFloor)) {
            switch (activeCam) {
        ...
        if (Input.GetKeyDown(KeyCode.KeypadEnter) && onLadder) {
    ...
    void getKeyInput()
    {
        ...
        if (activeCam.Equals(CamState.Top)) {
			...
			if ((Input.GetKey(KeyCode.UpArrow) || Input.GetKey(KeyCode.LeftArrow) || Input.GetKey(KeyCode.DownArrow) || Input.GetKey(KeyCode.RightArrow)) && !colliding)
            {
                if (sprinting)
                {
        ...
        else if (activeCam.Equals(CamState.First))
        {
			...
			if (Input.GetKey(KeyCode.DownArrow) && !checkCollisionBackwards())
            {
                gameObject.transform.position += gameObject.transform.forward * -movementSpeed * Time.deltaTime;
            } // else if
